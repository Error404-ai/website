...
function EventRegisterationForm() {
  ...
  const validateForm = () => {
    ...
    if (!studentNumber.trim() || !/^25\d{5,6}$/.test(studentNumber)) {
      tempErrors.studentNumber = "Student No. must start with '25' (2nd Year only) and be 7-8 digits.";
    }
    ...
    if (!rollNumber.trim() || !/^25\d{11}$/.test(rollNumber)) {
      tempErrors.rollNumber = "Roll No. must start with '25' (2nd Year only) and be 13 digits.";
